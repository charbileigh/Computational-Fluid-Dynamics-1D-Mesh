...
class Plotter:
    ...
    def Add1DPlot(self, iplot, title, Xlable, Ylabel, scale, dataPointMarkers, isplotLegend):
        ...
        :param scale: Either leave blank or set to "log" for a log scale
        ...
        if iplot <= 0:
            ...
            for idataPoint in dataPointMarkers:
                self.SubPlotYMarker.append(idataPoint)
    ...
    def Update2DPlotData(self, iplot, phi, variableName):
        ...
        if not (iplot > 0 and iplot <= self.mPlot):
            ...
            inode = 0
            for iy in range(0, len(coorX)):
                phiRow = []
                for ix in range(0, len(coorY)):
                    phiRow.append(phi[inode])
                    inode = inode + 1
